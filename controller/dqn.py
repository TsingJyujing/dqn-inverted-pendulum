...
class DQNController:
    ...
    def get_noise(self, tick: float):
        return random.gauss(0, 3) if self.greedy_epsilon(tick) else 0.0
    ...
    def step(self, tick: float, offset: float, velocity: float, theta: float, omega: float) -> float:
        ...
        F = self.force_set.numpy()[int(torch.argmax(result).numpy())] + self.get_noise(tick)
